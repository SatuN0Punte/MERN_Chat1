...
const Chat = () => ({
  const [online People, setOnline People] = useState({});
  const [offline People, setOffline People] = useState({});
  useEffect(() => {
  axios.get("/people").then((res) => {
  const offline PeopleArr = res.data
  .filter((p) => p._id != id)
  .filter((p) => lObject.keys(onlinePeople).includes (p._id)
  const offline People = {};
  offline PeopleArr.forEach((p) => {
  offlinePeople[p._id] = p;
  });
  setOffline People (offline People);
  });
  }, [onlinePeople]);
